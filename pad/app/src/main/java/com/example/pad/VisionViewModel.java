...
public class VisionViewModel extends ViewModel {
    // socket■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
    ArrayList<Integer> list=new ArrayList<>();    // 存放用户传入的方向
    ...
    // 1    2   3
    // 5    2.5 1.67
    public double[] distanceMap = { 5,2.5,1.67 }; // 对应不同的距离
    ...
    public VisionViewModel(){
        ...
        String []distance = {"1","2","3"};
        String []eye = {"左眼","右眼"};
        show_distance.setValue(distance[currentDistance] + "M");
    ...
    public void setCurrentDistance(int DistanceType){
        ...
        if (DistanceType>=0 && DistanceType<=2) {
            ...
            show_distance.setValue(distance[DistanceType] + "M");
        }
    }
    ...
    // 读入用户的手势
    public void add2List(int direction){
        list.add(direction);
    }
    ...
    public void generateAnswer(){
        // 通过分析用户的数据，得到最后的答案
        int up_cnt=0, down_cnt=0, left_cnt=0, right_cnt=0;
        int cur;
        ...
        int maxValue = Math.max(Math.max(Math.max(up_cnt,down_cnt),left_cnt),right_cnt);
        if (maxValue==0) checkAnswer(-1);
        else if (maxValue==up_cnt) checkAnswer(0);
        else if(maxValue==down_cnt) checkAnswer(1);
        else if(maxValue==left_cnt) checkAnswer(2);
        else if(maxValue==right_cnt) checkAnswer(3);
        else assert false;  // 报错
    }
